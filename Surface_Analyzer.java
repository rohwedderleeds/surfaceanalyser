...
public class Surface_Analyzer implements PlugIn {
...
//generate major protrusion table
                ...
                if (Math.abs(slope[helper]/revslope[helper])>sensitivity){
                    ...
                    if (slope[helper]<0){
                        ...
                        if (schalter < (protcount)){
                            ...
                            rt1.addValue("X position slice "+i, 0);
                            rt1.addValue("Y position slice "+i, 0);
                            rt1.addValue("Length "+i, 0);
                        }
                        schalter++;
            ...
            for (int x = 0; x < lang;x++){
                    if (peakmarker[x]>0){
                        peakcount++;
                        rt1.setValue(colcount,peakcount-1, x1[x]*pixres);
                        rt1.setValue(colcount+1,peakcount-1, y1[x]*pixres);
                        rt1.setValue(colcount+2,peakcount-1, radiusnew[x]*pixres);
                    }
            }
            if (majormax < peakcount){majormax = peakcount;}
            gross = rt1.size();
            for (int x = peakcount; x < gross; x++){
                rt1.setValue(colcount,x, 0);
                rt1.setValue(colcount+1,x, 0);
                rt1.setValue(colcount+2,x, 0);
            }
...
//calculate minor protrusions and generate table
            ...
            for (int minor = 0; minor < lang; minor++){
                if (revslope[minor]+slope[minor]<0){
                    ...
                    rt2.addValue("X position slice "+i, 0);
                    rt2.setValue(colcount,slopecount-1, x1[(int) Math.round(slopestart+(slopelength/2))]);
                    rt2.addValue("Y position slice "+i, 0);
                    rt2.setValue(colcount+1,slopecount-1, y1[(int) Math.round(slopestart+(slopelength/2))]);
                    rt2.addValue("Length "+i, 0);
                    rt2.setValue(colcount+2,slopecount-1, slopelength*pixres);
                }
            }
            colcount = colcount+3;
            filigross = rt2.size();
            if (minormax<slopecount){minormax=slopecount;}
//Output results
...
//show results tables
        for (int x = gross; x > majormax-1; x--){rt1.deleteRow(x);}
        for (int x = filigross; x > minormax-1; x--){rt2.deleteRow(x);}
